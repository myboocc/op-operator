...
<div id="page-content" class="clearfix page-content">
		...
		<form class="form-horizontal" role="form">
			...
			<table id="table_report" class="table table-striped table-bordered table-hover">
				...
				<tbody>
					<!-- 开始循环 -->	
					<c:choose>
						<c:when test="${not empty emailTemplateList}">
							<c:forEach items="${emailTemplateList}" var="email" varStatus="vs">
								<tr>
									<td class='center' style="width: 30px;">
										<label><input type="checkbox" name="ids" value="${email.id }"/><span class="lbl"></span></label>
									</td>
									<td><a>${email.name }</a></td>
									<td><a>${email.businessType }</a></td>
									<td><fmt:formatDate value="${email.updateTime}" type="both"/></td>
									<td style="width: 60px;">
										<div class='hidden-phone visible-desktop btn-group'>
											<a class='btn btn-mini btn-info' title="查看详细信息" onclick="findprovider('${email.id }');"><i class='icon-file'></i></a>
											<a class='btn btn-mini btn-info' title="编辑" onclick="editprovider('${email.id }');"><i class='icon-edit'></i></a>
											<a class='btn btn-mini btn-info' title="复制" onclick="editprovider('${email.id }');"><i class='icon-copy'></i></a>
										</div>
									</td>
								</tr>
							</c:forEach>
  ...
  <div class="modal fade" id="providerModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel">
	  <div class="modal-dialog" role="document">
		...
		//新增
		function add(){
			top.mainFrame.tabAddHandler('emailSubList','节假日新增','emailTemplate/goAddemail');
